...
The plugin ships a browser half (`exports["./client"]`, declared via `dsh.client.platform: "web"`), discovered by the client-modules scanner and loaded from the boot manifest. It injects one `<style>` tag with `@media (max-width: 700px)` overrides targeting the product's stable `data-slot` / `role` attributes, and registers capture-phase document listeners: `click` handles tap-outside collapse, collapse after an in-sidebar session action, the "+" button attach retarget, and the composer Enter tap/hold adjudication (collapse goes through the `layout` service's `toggleSidebar()`), `focusin` swallows the composer autofocus that follows a session switch; `keydown`/`keyup` implement tap-to-send and hold-to-newline with `enterkeyhint` set to send; it also watches `visualViewport` resize/scroll and mirrors the soft-keyboard inset into the `--mobilefix-kb` custom property consumed by rule 9. The style tag and all listeners are removed by the plugin's unload cleanup — fully reversible.
