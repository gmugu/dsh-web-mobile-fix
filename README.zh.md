...
插件带一个浏览器端（`exports["./client"]`，通过 `dsh.client.platform: "web"` 声明），由 client-modules 扫描器发现并随启动清单加载。它注入一个 `<style>` 标签，内容是针对产品稳定 `data-slot` / `role` / `aria-*` / `data-*` 语义属性的 `@media (max-width: 700px)` 覆盖；另注册 capture 阶段的 document 监听：`click` 负责点侧栏外收起与侧栏内选会话后收起（通过 `layout` 服务的 `toggleSidebar()`），`focusin` 负责吞掉切会话后落在输入框上的自动聚焦；并监听 `visualViewport` 的 resize/scroll，把软键盘高度镜像进 `--mobilefix-kb` 变量供规则 9 抬升外壳。样式标签与全部监听统一在插件卸载清理中移除——完全可逆。
